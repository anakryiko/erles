...
append(Pid, StreamId, ExpectedVersion, Events)
    when is_integer(ExpectedVersion), ExpectedVersion >= ?EXPECTED_VERSION_ANY,
         is_list(Events) ->
    gen_fsm:sync_send_event(Pid, {op, write_events, {StreamId, ExpectedVersion, Events, ?REQUIRE_MASTER}}).
...
transaction_start(Pid, StreamId, ExpectedVersion)
    when is_integer(ExpectedVersion), ExpectedVersion >= ?EXPECTED_VERSION_ANY ->
    gen_fsm:sync_send_event(Pid, {op, transaction_start, {StreamId, ExpectedVersion, ?REQUIRE_MASTER}}).

transaction_write(Pid, TransactionId, Events)
    when is_integer(TransactionId), TransactionId >= 0,
         is_list(Events) ->
    gen_fsm:sync_send_event(Pid, {op, transaction_write, {TransactionId, Events, ?REQUIRE_MASTER}}).

transaction_commit(Pid, TransactionId) when is_integer(TransactionId), TransactionId >= 0 ->
    gen_fsm:sync_send_event(Pid, {op, transaction_commit, {TransactionId, ?REQUIRE_MASTER}}).

delete(Pid, StreamId, any) ->
    delete(Pid, StreamId, ?EXPECTED_VERSION_ANY);
delete(Pid, StreamId, ExpectedVersion)
    when is_integer(ExpectedVersion), ExpectedVersion >= ?EXPECTED_VERSION_ANY ->
    gen_fsm:sync_send_event(Pid, {op, delete_stream, {StreamId, ExpectedVersion, ?REQUIRE_MASTER}}).

read_event(Pid, StreamId, last) ->
    read_event(Pid, StreamId, ?LAST_EVENT_NUMBER, false);
read_event(Pid, StreamId, EventNumber) when is_integer(EventNumber), EventNumber >= ?LAST_EVENT_NUMBER ->
    read_event(Pid, StreamId, EventNumber, false).
read_event(Pid, StreamId, last, ResolveLinks) when is_boolean(ResolveLinks) ->
...
read_event(Pid, StreamId, EventNumber, ResolveLinks)
    ...
    gen_fsm:sync_send_event(Pid, {op, read_event, {StreamId, EventNumber, ResolveLinks, ?REQUIRE_MASTER}}).
...
read_stream_forward(Pid, StreamId, FromEventNumber, MaxCount, ResolveLinks)
    when is_integer(FromEventNumber), FromEventNumber >= 0,
         is_integer(MaxCount), MaxCount > 0,
         is_boolean(ResolveLinks) ->
    gen_fsm:sync_send_event(Pid, {op, read_stream_events_forward, {StreamId, FromEventNumber, MaxCount, ResolveLinks, ?REQUIRE_MASTER}}).

read_stream_backward(Pid, StreamId, last, MaxCount) when is_integer(MaxCount), MaxCount > 0 ->
...
read_stream_backward(Pid, StreamId, FromEventNumber, MaxCount, ResolveLinks)
    when is_integer(FromEventNumber), FromEventNumber >= ?LAST_EVENT_NUMBER,
         is_integer(MaxCount), MaxCount > 0,
         is_boolean(ResolveLinks) ->
    gen_fsm:sync_send_event(Pid, {op, read_stream_events_backward, {StreamId, FromEventNumber, MaxCount, ResolveLinks, ?REQUIRE_MASTER}}).

read_all_forward(Pid, first, MaxCount) when is_integer(MaxCount), MaxCount > 0 ->
...
read_all_forward(Pid, FromPos={tfpos, CommitPos, PreparePos}, MaxCount, ResolveLinks)
    when is_integer(CommitPos), CommitPos >= 0, is_integer(PreparePos), PreparePos >= 0,
         is_integer(MaxCount), MaxCount > 0,
         is_boolean(ResolveLinks) ->
    gen_fsm:sync_send_event(Pid, {op, read_all_events_forward, {FromPos, MaxCount, ResolveLinks, ?REQUIRE_MASTER}}).

read_all_backward(Pid, last, MaxCount) when is_integer(MaxCount), MaxCount > 0 ->
...
read_all_backward(Pid, FromPos={tfpos, CommitPos, PreparePos}, MaxCount, ResolveLinks)
    when is_integer(CommitPos), CommitPos >= ?LAST_LOG_POSITION, is_integer(PreparePos), PreparePos >= ?LAST_LOG_POSITION,
         is_integer(MaxCount), MaxCount > 0,
         is_boolean(ResolveLinks) ->
    gen_fsm:sync_send_event(Pid, {op, read_all_events_backward, {FromPos, MaxCount, ResolveLinks, ?REQUIRE_MASTER}}).
